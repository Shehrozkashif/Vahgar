...
import opimmmod._
class immgenrmod extends Module {
  ...
  switch(opcode) {
    ...
    is(S) {
      io.imm := Cat(Fill(20, io.instruction(31)), io.instruction(31, 24), io.instruction(11, 7))
    }
    is(B) {
      io.imm := Cat(Fill(19, io.instruction(31)),  io.instruction(7), io.instruction(30, 25),io.instruction(11, 8),0.U)
    }
    is(U) {
